...
def plot_convergence(loss_std, loss_qat, loss_ptq_final): 
...
def train_qat_with_gradient_scaling(X, y, lr, total_bits, frac_bits, scaling_factor):
    print("\n--- Training with Gradient Scaling ---")
    model_qat = LinearRegression()
    loss_qat = model_qat.fit_normal_descent_quantize_gradient_scaling(X, y, epochs=500, lr=lr, total_bits=total_bits, frac_bits=frac_bits, scaling_factor=scaling_factor)  
    return loss_qat


def train_baseline_model(X, y, lr):
    print("--- Training Baseline Model (FP64) ---")
    model_std = LinearRegression()
    loss_std = model_std.fit_gradient_descent(X, y, epochs=500, lr=lr)
...
if __name__ == "__main__":
    ...
    model_std, loss_std = train_baseline_model(X, y, LEARNING_RATE)
    ...
    loss_qat_gradient_scaling = train_qat_with_gradient_scaling(X, y, LEARNING_RATE, TOTAL_BITS, FRAC_BITS, scaling_factor=1.05)  # You can experiment with different scaling factors like 10.0, 20.0, etc.

    #Visualize the convergence of the three methods
    plot_convergence(loss_std, loss_qat_gradient_scaling, loss_ptq_final)